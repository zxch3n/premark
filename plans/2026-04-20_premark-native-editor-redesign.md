...
- Active marker reveal now has an explicit editable-run sidecar: every source-addressable visible run carries `revealedOffsets -> sourceOffsets` boundaries. Editable layout should use those runs first and treat old segment interpolation only as compatibility fallback.
- Canvas text drawing must use the same grapheme boundary model as editable geometry for emoji-like runs. A single `fillText` call can accumulate different emoji advances than the layout/editor boundary table, so repeated ZWJ emoji drift even when DOM rendering looks correct.
...
- Reworked active marker reveal source mapping to the more explicit editable-run architecture. `ActiveMarkerRevealSourceMap` now includes `runs`, each with raw revealed boundaries and source boundaries; `EditableLayoutIndex` uses those runs before falling back to legacy segment interpolation. This removes the need for the previous ad hoc boundary normalization when control text is escaped or merged with normal text in one rendered fragment.
- Added tests that assert the run sidecar itself, not only final caret geometry, for strong markers and link suffix controls.
- Verification after the editable-run refactor: full `vp test` passes 153 tests, `vp run build` passes, `vp run test:browser` passes 18 Playwright tests, and targeted `vp test packages/editor/tests/marker-reveal.test.ts packages/editor/tests/editable-layout.test.ts` passes 34 tests. `vp check --fix` still has the two existing wiki-canvas warnings.
- Fixed Canvas-only drift for repeated family emoji. Root cause: editable caret/hit-test geometry used Premark/pretext grapheme boundaries, while Canvas painted an emoji-containing text fragment with one `fillText` run whose emoji advances accumulated differently. Canvas drawing now splits emoji-like runs by grapheme and positions each cluster at the layout boundary; plain text still draws as a single run to preserve normal kerning.
