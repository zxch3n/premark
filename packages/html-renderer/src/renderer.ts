...
function renderFragment(fragment: InlineFragment, leadingGap: number): string {
  const className = `pmd-fragment pmd-fragment--${fragment.type}`;
  const inlineStyle = style({
    "margin-left": leadingGap > 0.5 ? `${leadingGap}px` : undefined,
    font: fragment.font,
  });
...
function renderTextLine(line: TextLine, block: BlockLayout): string {
  const fragments = line.fragments
    .map((fragment, index) => {
      ...
      const leadingGap =
        previous === undefined ? fragment.x : fragment.x - previous.x - previous.width;
      return renderFragment(fragment, leadingGap);
    })
    .join("");
